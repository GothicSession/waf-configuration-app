...
function _M.filter()
    ...
    if WafServiceConfig.configs["proxy_pass_enable"] ~= true then
        return
    end

    local matcher_list = WafServiceConfig.configs['matcher']
    local upstream_list = WafServiceConfig.configs['backend_upstream']

    for i, rule in ipairs( WafServiceConfig.configs["proxy_pass_rule"] ) do

...
function _M.verify_javascript()
    ...
    if _M.verify_javascript_html == nil then
        local path = WafServiceConfig.home_path() .."/support/verify_javascript.html"
        f = io.open( path, 'r' )
        if f ~= nil then
...
function _M.filter()
    if WafServiceConfig.configs["browser_verify_enable"] ~= true then
        return
    end

    local matcher_list = WafServiceConfig.configs['matcher']
    for i,rule in ipairs( WafServiceConfig.configs["browser_verify_rule"] ) do

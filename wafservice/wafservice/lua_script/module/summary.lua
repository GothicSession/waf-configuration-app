...
function _M.refresh()
    local period = tonumber( WafServiceConfig.configs["summary_request_enable"] )
...
function _M.pre_run_matcher()
    if WafServiceConfig.configs["summary_request_enable"] ~= true or WafServiceConfig.configs["summary_collect_enable"] ~= true then
        return
    end

    local matcher_list = WafServiceConfig.configs['matcher']
    for i,rule in ipairs( WafServiceConfig.configs["summary_collect_rule"] ) do
...
function _M.log()
    ...
    if WafServiceConfig.configs["summary_request_enable"] ~= true then
        return
    end

    local with_host_info = WafServiceConfig.configs["summary_with_host"]
    ...
    if WafServiceConfig.configs["summary_group_persistent_enable"] == true then
    ...
    if WafServiceConfig.configs["summary_group_temporary_enable"] == true then

...
def install_openresty( ):
    if os.path.exists('/opt/WafService/WafService') == True:
    ...
    # May Not Work in CI
    # exec_sys_cmd( 'export PATH=/opt/wafservice/openresty/nginx/sbin:$PATH' )
...
def install_wafservice():
    ...
    #install WafService file
    print('### copy WafService files ...')
    os.chdir( work_path )
    if os.path.exists('/opt/wafservice/') == False:
        exec_sys_cmd( 'mkdir -p /opt/wafservice' )

    exec_sys_cmd( 'cp -r -f ./wafservice /opt/wafservice' )

    #copy nginx config file to openresty
    if os.path.exists('/opt/wafservice/openresty') == True:
        if filecmp.cmp( '/opt/wafservice/openresty/nginx/conf/nginx.conf', '/opt/wafservice/openresty/nginx/conf/nginx.conf.default', False ) == True:
    ...
    # file too open;
    # exec_sys_cmd( 'chmod -R 777 /opt/wafservice/wafservice/configs' )
    exec_sys_cmd( 'chmod -R 755 /opt/wafservice/wafservice/configs' )
